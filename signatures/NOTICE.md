...
The shipped `bundle.psenc` contains a merged, compile-verified collection of
YARA rules aggregated from three upstream open-source repositories. The
plaintext `rules/bundled.yar` is a build intermediate regenerated by
`tools/build-rules.sh`; it is not shipped. The bundle was produced
...
Total: 875 source files, 13,101 effective rules.

## GPL-2.0 obligation (important)

Because this bundle includes rules from `Yara-Rules/rules`, which is licensed
under **GPL-2.0**, the combined rule bundle is a derivative work
...
2. Make the corresponding source form of the bundle available to recipients
   through the reproducible build pipeline and pinned provenance.
